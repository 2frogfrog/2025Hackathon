...
add_button_rect = pygame.Rect(10, 20, 30, 30)
...
def draw_ui():
    ...
    for rect, user_text, placeholder, active in [
        ...
        if user_text:
            color = BLACK
            text_to_draw = user_text
        else:
            color = GRAY
            text_to_draw = placeholder
        screen.blit(font.render(text_to_draw, True, color), (rect.x + 5, rect.y + 5))
    ...
    for i, item in enumerate(items):
        ...
        item_amount_text = f"{item.current}/{item.needed} needed"
        screen.blit(font.render(item_amount_text, True, BLACK), (150, y))

        for b in ["minus", "plus", "delete"]:
            pygame.draw.rect(screen, LIGHT_GRAY, rect[b])
            symbol = "-" if b == "minus" else "+" if b == "plus" else "X"
            screen.blit(font.render(symbol, True, BLACK), (rect[b].x + 10, rect[b].y + 5))
